...
def connect_payment_device(request, payment):
    ...
    Three cases, matched exactly to the spec:
      ...
      C) old_mac != new_mac       -> bypass new_mac FIRST (priority), then
                                      unbypass old_mac (secondary cleanup)
    ...
    with transaction.atomic():
        ...
        # PRIORITY: get the NEW device online first. bypass_mac is what
        # actually grants internet - queued and run by the agent before
        # anything about the old device, so the waiting customer is never
        # delayed by cleanup work for a different device.
        if router:
            ...
            if not mikrotik_profile or mikrotik_profile.router_id != router.id:
                ...
                try:
                    ...
                    if job and job.status == MikroTikJob.Status.FAILED and not warning:
        ...
        # SECONDARY: release the old device, strictly after the new
        # device's bypass job is already ahead of it in the queue. Case A
        # (no old_mac) and Case B (old_mac == new_mac) skip this entirely.
        ...
        return warning
def connect_voucher_device(request, voucher):
    """
    Voucher-equivalent of connect_payment_device   identical three-case
    ownership logic, but scoped to a Voucher instead of a Payment, since
    vouchers bypass M-Pesa entirely and have no Payment row at all. Kept
    as a fully separate function (not a generalization of
    connect_payment_device) so the M-Pesa path can never be affected by
    anything here.
    """
    from django.db import transaction
    from django.utils import timezone
    ...
    if not new_mac:
        return ("Your voucher is valid and your time is reserved, but we "
                "couldn't detect your device's MAC address. Please reconnect "
                "to the Wi-Fi hotspot and open the voucher page again from there.")

    with transaction.atomic():
        voucher = type(voucher).objects.select_for_update().get(pk=voucher.pk)
        old_mac = voucher.mac_address

        subscription = Subscription.objects.filter(voucher=voucher).order_by('-created_at').first()
        if not subscription:
            return "Your voucher is valid, but no active package was found to attach this device to."
        ...
        if router:
            ...
            if not mikrotik_profile or mikrotik_profile.router_id != router.id:
                warning = (
                    f"Your voucher is valid and your time is reserved, but the "
                    f"\u201c{subscription.package.name}\u201d package has no MikroTik "
                    f"profile configured for this router."
                )
            else:
                try:
                    get_mikrotik_service(router).bypass_mac(
                        new_mac, comment=f'voucher{voucher.id} until {subscription.expiry_time}',
                    )
                    ...
                    warning = ("Your voucher is valid and your time is reserved, but we "
                               "couldn't reach the router to get you online just now. "
                               "Try again in a minute, or contact support.")
        else:
        ...
        if voucher.mac_address != new_mac:
            ...
            voucher.save(update_fields=['mac_address'])

        # SECONDARY: release the old device now, strictly after the new
        # one's bypass job is already ahead in the queue.
        if old_mac and old_mac != new_mac:
            old_session = InternetSession.objects.filter(
                voucher=voucher, mac_address=old_mac, status=InternetSession.Status.ACTIVE,
            ).first()
            if router:
                service = get_mikrotik_service(router)
                try:
